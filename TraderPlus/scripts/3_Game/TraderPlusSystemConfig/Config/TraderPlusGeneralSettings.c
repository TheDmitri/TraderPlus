class TraderPlusGeneralSettings
{
	...
	void CheckForUpdate()
	{
		...
		{
			vector pos = Traders[i].Position;
			Traders[i].Position[0] = Math.Round(pos[0]);
			Traders[i].Position[1] = Math.Round(pos[1]);
			Traders[i].Position[2] = Math.Round(pos[2]);

			vector ori = Traders[i].Orientation;
			Traders[i].Orientation[0] = Math.Round(ori[0]);
