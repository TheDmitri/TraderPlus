class GarageMenu extends UIScriptedMenu
{
  ...
  void UIHandle()
  {
    ...
    m_CarInPark = GarageHelper.CheckForCarAtPos(m_ParkingPos);
    if(m_CarInPark)
    {
      #ifdef GMDEBUG
  ...
  void ParkInHandler()
  {
      ...
      {
        if(!GarageHelper.CanParkVehicle(m_CarInPark))return;
      }
      ...
      GetRPCManager().SendRPC("Garage", "ParkInRequest",  new Param3<int, ref CarScript, vector>(m_LowUID, m_CarInPark,m_ParkingPos), true, NULL);
