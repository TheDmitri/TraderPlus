...
class CarLockMenu extends UIScriptedMenu
{
  ...
  void CarLockMenu(ref CarScript car)
  {
    m_Car = car;
  }
  ...
  void PasswordHandler()
  {
    ...
    if(passwordAttempt && m_Car.m_CarLockOwner == -1 || (m_Car.m_CarLockOwner==player.CLSteamlowID && m_Car.m_CarLockPassword == -1))
    {
      ...
      GetRPCManager().SendRPC("CarLock", "CarLockPasswordRequest",  new Param2<ref CarScript, int>(m_Car,passwordAttempt), true, NULL);
    ...
    {
      GetRPCManager().SendRPC("CarLock", "CarLockRequest",  new Param2<ref CarScript, bool>(m_Car,false), true, NULL);
