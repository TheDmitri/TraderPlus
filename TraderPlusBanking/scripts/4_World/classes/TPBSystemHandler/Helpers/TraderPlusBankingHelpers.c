class TraderPlusBankHelpers
{
	static void SendBankAccountInfo(PlayerBase player)
	{
		...
		#endif
		GetRPCManager().SendRPC("TraderPlusBanking", "GetTraderPlusBankingBankAccount",  new Param1<ref TraderPlusBankingData>(player.m_BankAccount), true, player.GetIdentity());
		player.m_BankAccount.Save(player.GetIdentity().GetPlainId());
	}
	...
	static void SetAmountBankAccount(PlayerBase player, int amount)
	{
		...
		#endif
		player.m_BankAccount.MoneyAmount = amount;
		SendBankAccountInfo(player);
	}
