class TraderPlusHelper
{
  ...
  static void EmptyMag(PlayerBase player, Magazine mag)
    ...
    {
      ref array<Magazine> items = new array<Magazine>;
      string item_name;
      ...
      if(stack_size > 0)
      {
          ...
          {
              pile = Magazine.Cast(CreateInInventory(player,item_name,stack_size));
              items.Insert(pile);
              SetHealthFromDamage(pile, bulletsDamage);
          }
          if ( rest > 0)
          {
              pile = Magazine.Cast(CreateInInventory(player,item_name, rest));
              items.Insert(pile);
              SetHealthFromDamage(pile, bulletsDamage);
          }
      }
  ...
  static bool CheckStock(string name, int health, int i)
	{
		for(int j=0; j<GetTraderPlusServer().TraderStocks.Get(i).TraderPlusItems.Count();j++)
		{
			...
			string line = GetTraderPlusServer().TraderStocks.Get(i).TraderPlusItems.Get(j);
			line.Split(" ",token);
			if(token[0] == name && token[2] == health.ToString())
			{
				return true;
			}
			token.Clear();
...
static bool IsWearByPlayer(InventoryLocation il)
{
  ...
  static bool IsVehicle(string classname)
  {
    ...
    if(GetGame().IsServer())
    {
      for(int j=0;j<GetTraderPlusVehiculeConfig().VehiclesParts.Count();j++)
      {
        string lowVehicleName = GetTraderPlusVehiculeConfig().VehiclesParts.Get(j).VehicleName;
        lowVehicleName.ToLower();
        if(lowClassName==lowVehicleName)return true;
      }
      return false;
    }
    else
    {
      ...
      for(int i=0;i<GetTraderPlusConfigClient().VehicleNames.Count();i++)
      {
        string lowerVehicleClientName = GetTraderPlusConfigClient().VehicleNames.Get(i);
        lowerVehicleClientName.ToLower();
        if(lowerVehicleClientName == lowClassName) return true;
      }
      return false;
    }
    return false;
  ...
  static int GetIDBasedOnClassName(string classname)
  {
    string lowClassName = classname;
    lowClassName.ToLower();
    ...
    for(int i=0;i<GetTraderPlusVehiculeConfig().VehiclesParts.Count();i++)
    {
      ...
      {
        #ifdef TRADERPLUSDEBUG
        GetTraderPlusLogger().LogInfo("GetIDBasedOnClassName: ID found:"+i.ToString());
        #endif
        return i;
      }
    }
  ...
  static bool AddVehicleToGarage(PlayerBase player, string classname)
  {
    ...
    if (obj)
    {
      ...
      {
          Object attchObj = GetGame().CreateObject(vehicleparts[i], vector.Zero, false, false, false);
          EntityAI attch = EntityAI.Cast(attchObj);
          if (attch)
          {
            itemsArr.Insert(new ItemsCargo(attch.GetType(), GetItemAmount(attch), attch.GetHealth(),GARAGEVEHICLEATTACHMENT));
            GetGame().ObjectDelete(attch);
          }
      }
      string id = GarageHelper.GetLowSteamID(player.GetIdentity().GetPlainId()).ToString();
      ...
      if(garageData)
      {
        ...
        foreach(VehiculeData car: garageData.Vehicles)
        {
          ...
          vehiclesHealth.Insert(GarageHelper.ConvertHealthToHealthLevel(car.VehicleName, car.EngineHealth));
        }
        GetRPCManager().SendRPC("Garage", "GarageResponse",  new Param4<ref TStringArray, ref TIntArray,vector, int>(vehiclesName, vehiclesHealth, vector.Zero, TraderPlusResponse.BUY_SUCCESS),true, player.GetIdentity());
        return true;
      }
    }
    return false;
  ...
  static bool CreateVehicle(PlayerBase player, string classname, vector position, vector orientation)
	{
      ...
      if(vehicle)
      {
        ...
        vehicleparts = GetVehicleParts(classname);
    		...
    		for (int i = 0; i < vehicleparts.Count(); i++)
    		{
    			Object attchObj = GetGame().CreateObject(vehicleparts.Get(i), vector.Zero, false, false, false);
    			EntityAI attch = EntityAI.Cast(attchObj);
    			if (attch)
          {
            #ifdef TRADERPLUSDEBUG
            GetTraderPlusLogger().LogInfo("Create vehicle => ");
            #endif
            vehicle.GetInventory().CreateAttachment(vehicleparts.Get(i));
          }
          #ifdef TRADERPLUSDEBUG
          GetTraderPlusLogger().LogInfo("GetVehicleParts => delete attach");
          #endif
    			GetGame().ObjectDelete(attch);
    		}
  ...
  static bool IsProductVehicle(string classname)
  {
    string lowClassName = classname;
    lowClassName.ToLower();
  ...
  static bool CreateInInventoryWithState(PlayerBase player, string classname,int state = 0, int quantity=1)
  {
    string oldClassname="";
    ...
    if( IsProductVehicle(classname) )
    {
      if(GetTraderPlusConfigServer().UseGarageToTradeCar){
        return AddVehicleToGarage(player, classname);
      }
    ...
    ItemBase item = CreateInInventory(player,classname,quantity);
    if(item && !IsVehicle)
    {
    ...
    {
        Receipt carBox = Receipt.Cast(item);
        carBox.SetHealth("","",GetDamageStateFromLevel(state)*item.GetMaxHealth());
        int id = GetIDBasedOnClassName(oldClassname);
  ...
  static ItemBase CreateInInventory(PlayerBase player, string classname, int quantity = 1)
  {
      ...
      if(quantity > maxQuantity)
      {
        ...
        CreateInInventory(player, classname, remainingqty);
      }
